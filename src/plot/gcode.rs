...
use crate::{
    plot::model::{
        MotionKind, MotionSegment, PrintStartMode, ToolSettings, ToolpathPlan, ToolpathStats,
    },
...
pub fn build_plan(prepared: PreparedSvg, settings: &ToolSettings) -> ToolpathPlan {
    ...
    let first_draw_point = prepared
        .polylines
        .iter()
        .find(|polyline| polyline.len() >= 2)
        .and_then(|polyline| polyline.first().copied());
    ...
    for polyline in prepared.polylines.iter().filter(|polyline| polyline.len() >= 2) {
        ...
        let start_draw = vec3(polyline[0].x, polyline[0].y, 0.0);
...
mod tests {
    ...
    use super::*;
    use crate::plot::model::{PrintStartMode, PrintableArea, ToolSettings};
