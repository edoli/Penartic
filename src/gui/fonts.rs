...
use std::{collections::HashSet, fs, sync::mpsc, thread};

pub type LoadedFallbackFonts = Vec<(String, FontData)>;
...
fn collect_fallback_font_sources() -> Vec<(String, PathBuf)> {
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(dir) = exe_path.parent() {
            let custom_font_path = dir.join("fallback_font.ttf");
            if custom_font_path.is_file() {
                sources.push(("user_fallback".to_owned(), custom_font_path));
            }
        }
    }
...
fn append_platform_cjk_font_sources(sources: &mut Vec<(String, PathBuf)>) {
    sources.extend([
        ...
        ("cjk_zh_pingfang".to_owned(), PathBuf::from("/System/Library/Fonts/PingFang.ttc")),
        (
...
#[cfg(not(target_arch = "wasm32"))]
pub fn spawn_fallback_font_loader() -> Option<mpsc::Receiver<LoadedFallbackFonts>> {
    ...
    thread::spawn(move || {
        ...
        for (font_key, font_path) in sources {
            ...
            match fs::read(&font_path) {
                ...
                Err(error) => {
                    eprintln!("failed to read fallback font '{}': {error}", font_path.display());
                }
            }
        }
...
pub fn spawn_fallback_font_loader() -> Option<std::sync::mpsc::Receiver<LoadedFallbackFonts>> {
    None
}
