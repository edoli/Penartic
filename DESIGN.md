...
7. Open the generated G-code in a dedicated viewer when needed; the viewer shows line numbers for inspection while copy/save actions still use the raw G-code text, and the job can also be copied to the clipboard or saved as a `.gcode` file.
...
- small compensated cutter turns stay as contiguous line geometry instead of becoming explicit swivel arcs so the downstream G2/G3 fitter can still compress smooth drag-knife motion without losing true sharp-corner swivels
...
- G2/G3 arc output is optional because firmware support varies and is used for rounded-corner transitions, drag-knife corner-swivel arcs, and whole-stroke polyline arc compression, reducing overly segmented motion while keeping whole-stroke fitting bounded by geometric-error checks against the original polyline; drag-knife compensation avoids materializing tiny swivel arcs so smooth compensated curves still reach the whole-stroke fitter
